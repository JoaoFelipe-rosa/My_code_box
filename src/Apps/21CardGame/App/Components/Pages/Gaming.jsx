...
function StartGame({
  nextStep, setGameMode, pile, loading,
}) {
  const cards = useMemo(() => {
    if (pile) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {loading ? <button type="button" className="p-2 w-1/3 rounded transition ease-in-out delay-150 bg-white hover:-translate-y-1 hover:scale-110 hover:bg-slate-200 duration-300" onClick={() => (setGameMode(nextStep))}>Start Game</button> : <> </>}
    </div>
  );
}
...
function GameStep({
  onClickPile, setGameMode, pile, nextStep,
}) {
  return (
...
export default function Gaming() {
  ...
  const RenderGameMode = useCallback(() => {
    switch (gameMode) {
      case 'STEP0': return <StartGame setGameMode={setGameMode} pile={pile} nextStep="STEP1" loading={loading} />;
      case 'STEP1': return <GameStep onClickPile={onClickPile} setGameMode={setGameMode} pile={pile} nextStep="STEP2" />;
      case 'STEP2': return <GameStep onClickPile={onClickPile} setGameMode={setGameMode} pile={pile} nextStep="STEP3" />;
      case 'STEP3': return <GameStep onClickPile={onClickPile} setGameMode={setGameMode} pile={pile} nextStep="STEP4" />;
      case 'STEP4': return <Result pile={pile} />;
      default: return (
        <div className="flex flex-col items-center">
